...
#include "app.h"
#include "C:\Users\gcurry\Documents\GitHub\GaleCurry_ME433_2016\HW_6_IMU.X\LCD.h"
#include <math.h>
int txFlag; 
int pos;
char tx[100];
            ...
            if(appData.isReadComplete == true)
            {
                ...
                for(ii=0; ii<appData.readLength; ii++) {
                    if (appData.readBuffer[ii] == '\n' || appData.readBuffer[ii] == '\r'){
                        ...
                        float scale = 2;
//                        if (qq>=0){
//                            OC2RS = min_val + floor(qq*(float)(diff_max/100.0)); //duty cycle = OC1RS/2000, qq ranges from 0-232ish
//                            OC1RS = min_val;
//                        }                   
//                        else if (qq<0){
//                            OC1RS = min_val + floor(-qq*(float)(diff_max/100.0)); 
//                            OC2RS= min_val;
//                        } 
                        if (qq>320){
                            diff = qq - 320;
                            OC2RS = min_val +floor((float)(diff*scale));
                            OC1RS = min_val;
                        }
                        if (qq<320){
                            diff = 320 - qq;
                            OC1RS = min_val + floor((float)(diff*scale));
                            OC2RS = min_val; 
                        }
                    }
                    else {
